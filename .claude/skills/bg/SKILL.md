...
- `git checkout -b {ブランチ名} origin/develop --no-track`

### 3. 関連コードを調査して実装する

- Issueの内容から影響範囲を特定し、関連ファイルを読む
- `CLAUDE.md` と `docs/plan/loverbleでの実装方式.md`、`docs/requirements/` を必ず確認
...
- コミットメッセージのルール:
  - 一行で簡潔に（50文字以内を目安）
  - prefixを必ず付ける（`feat:`, `fix:`, `refact:`, `docs:`, `chore:` 等）
  - 日本語で書く
  - 末尾に `(#{番号})` を付ける
...
- `gh pr create` でPR作成:
  - `--repo ShotaGhoona/archaive-quote-demo-v2`
  - `--base develop`

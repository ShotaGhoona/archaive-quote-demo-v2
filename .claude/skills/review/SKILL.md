...
1. **PRの情報を取得する**
   ```
   gh pr view {番号} --repo ShotaGhoona/archaive-quote-demo-v2
   gh pr diff {番号} --repo ShotaGhoona/archaive-quote-demo-v2
   ```
...
3. **以下の観点でレビューする**
   ...
   **品質**
   - 既存のコードスタイル・パターンと一貫しているか（CLAUDE.md / docs/plan/loverbleでの実装方式.md 参照）
...
5. **特定の行にコメントが必要な場合**
   ```
   gh api repos/ShotaGhoona/archaive-quote-demo-v2/pulls/{番号}/comments \

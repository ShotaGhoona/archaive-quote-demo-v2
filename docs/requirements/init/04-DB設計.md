...
CREATE TABLE public.variable_definitions (
  ...
  -- MASTER の場合
  source_entity TEXT,        -- 'MATERIAL' / 'PROCESS' / 'CUSTOMER'
  source_field TEXT,         -- 'unit_price' / 'attributes->density' 等
  -- LOOKUP の場合
...
CREATE TABLE public.selection_tree_nodes (
  ...
  -- 役割 A: ナビゲーション（必須）。どのノードマスタの値か
  node_master_value_id UUID REFERENCES public.node_master_values(id) ON DELETE SET NULL,

  -- 役割 B: マスタ参照（オプション）
  master_entity TEXT,           -- 'MATERIAL' / 'PROCESS' / 'CUSTOMER' / NULL
  master_id UUID,                -- 該当マスタの ID（FK 制約は付けない、polymorphic）

  -- 役割 C: 固定変数（オプション）
  fixed_variables JSONB,         -- { "係数": 1.5, ... }

  -- 役割 D: 計算式テンプレート（葉の場合のみ必須）
...
CREATE TABLE public.materials (
  ...
  attributes JSONB DEFAULT '{}',     -- { "比重": 7.85, "硬度": ... }
  remarks TEXT,
  created_at TIMESTAMPTZ DEFAULT now(),
  updated_at TIMESTAMPTZ DEFAULT now()
);
...
selection_tree_nodes ────────────────────────────────────────────┘
  ├─ formula_template_id (葉のみ)
  ├─ node_master_value_id (役割 A)
  ├─ master_id (役割 B、polymorphic: materials/processes/customers)
  └─ fixed_variables JSONB (役割 C)

quote_formula_settings (シングルトン)
```
...
20260501000005_create_node_masters.sql                -- ノードマスタ
20260501000006_create_materials_and_processes.sql    -- 材料・工程マスタ
...
- **FK の polymorphic 部分**（`selection_tree_nodes.master_id` など）は FK 制約を付けない代わりに、アプリケーション側で整合性を保つ
